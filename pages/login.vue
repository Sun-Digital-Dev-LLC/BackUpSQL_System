<template>
    <div class="flex items-center justify-center min-h-screen">
      <div class="p-8 bg-white rounded-lg shadow-md">
        <h1 class="mb-6 text-2xl font-bold text-center">登入 Login</h1>
        <button
          v-if="!session"
          @click="handleLogin"
          class="flex items-center justify-center w-full px-4 py-2 space-x-2 text-white bg-[#5865F2] rounded hover:bg-[#4752C4] transition-colors"
        >
        <svg
          class="w-6 h-6"
          fill="currentColor"
        ...
        >
          ...
          <span class="font-semibold">使用 Discord 帳號登入</span>
        </button>
      </div>
    </div>
  </template>
  ...
  const handleLogin = async () => {
    try {
      await signIn('discord', { callbackUrl: '/client/dashboard' })
    } catch (error) {
